namespace Panda.Services                        
{
    ...
    public class UserService : IUserService
    {
        ...
        public string GetUserId(UserLoginInputModel input)
        {
            var user = this.dbContext.Users
                .Where(u => u.Username == input.Username
                        && u.Password == ComputeHash(input.Password))
                .FirstOrDefault();
        ...
        public UsernameViewModel GetUsernameById(string userId)
        {
            var username = new UsernameViewModel
            { 
                Username = this.dbContext.Users.FirstOrDefault(u => u.Id == userId).Username,
            };
